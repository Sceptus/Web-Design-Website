...
<body>
    <h1>MTHS CS Polls</h1>
    <hr />
    <a href="//mthscs.net/vsenthil/polls/checkpoint-8">Home</a>
    &nbsp;&nbsp;|&nbsp;&nbsp;
    <a href="//mthscs.net/vsenthil/polls/checkpoint-8/create.php">Create Poll</a>
    <br />
    <hr />
    <br />
    ...
    <?php
        ...
        $ip_query = "SELECT ip_address FROM poll_results WHERE poll_id='$id'";
        $ip_result = mysqli_query($connect, $ip_query);

        while($row = mysqli_fetch_assoc($ip_result)) {
            if($ip_address == $row["ip_address"]) {
                echo "Sorry! You have already voted from: ". $ip_address;
                exit();
            }
        }
